...
def resizeAndPutText(file_list, sw_tag, sw_date, w=1920, h=1080, splitSize=3, textSize=0.7, tx=520, ty=346):
    global namePattern
...
def imagesToMp4(retList):
    cmd = ""
    cmd += 'ffmpeg -loglevel fatal -y -loop 1 -t 10 -i %s -loop 1 -t 10 -i %s -loop 1 -t 10 -i %s -loop 1 -t 10 -i %s -loop 1 -t 10 -i %s ' % (retList[0], retList[1], retList[2], retList[3], retList[4])
    cmd += ' -loop 1 -t 10 -i %s -loop 1 -t 10 -i %s -loop 1 -t 10 -i %s -loop 1 -t 10 -i %s -loop 1 -t 10 -i %s ' % (retList[5], retList[6], retList[7], retList[8], retList[9])
    ...
    cmd += '[v0][v1][v2][v3][v4][v5][v6][v7][v8][v9]concat=n=10:v=1:a=0,format=yuv420p[v]" -map "[v]" %s' % ('./' + "out1.mp4")
    os.system(cmd)
